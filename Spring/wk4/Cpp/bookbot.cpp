...
bool BookBot::PlaceDownBook(BookPile& pile) {
    ...
    pile.AddBook(this->heldBook.second);
    this->heldBook = EMPTY_BOOK_OPT;
}
...
TEST_CASE( "BookBot place to pile", "[bookbot]" ) {
    ...
    p.ListAllBooks(ss2);
    REQUIRE( ss2.str().find("\"Foo\" by Bar") != std::string::npos );
}
...
TEST_CASE( "BookBot cannot hold multiple books", "[bookbot]" ) {
    ...
    std::stringstream ss1;
    bb.ListContents(ss1);
    REQUIRE( ss1.str() == "\"Baz\" by Bar\n" );
    std::stringstream ss2;
    p.ListAllBooks(ss2);
    REQUIRE( ss2.str().find("\"Foo\" by Bar\n") != std::string::npos );
}
